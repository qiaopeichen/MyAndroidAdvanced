/*
	...
	要求细粒度对象，那么不可避免地使得对象数量多且性质相近。这些对象分为两个部分：内部状态和外部状态。内部状态是对象可共享出来的信息，存储在享元对象内部并且不会随环境的改变而改变；
...
/*
	享元模式的简单实现
	某著名网上商城卖商品，如果每个用户下单都生成商品对象，这显然会耗费很多资源。如果赶上双11，那恐怖的订单量会生成很多商品对象，就极易产生OutOfMemory。
	因此我们采用享元模式来对商品的创建进行优化。
*/
...
/*
	2.具体享元角色
	定义类Goods，它实现IGoods接口，并实现了showGoodsPrice方法，如下所示：
*/
...
/*
	其中name为内部状态，version为外部状态。
	showGoodsPrice方法根据version的不同会打印出不同的价格。
*/
...
/*
	享元工厂GoodsFactory用来创建Goods对象。通过Map容器来存储Goods对象，将内部状态name作为Map的key，以便标识Goods对象。
	如果Map容器中包含此key，则使用Map容器中存储的Goods对象；否则就新创建Goods对象，并放入Map容器中。
*/
...
/*
	...
	从输出可以看出，只有第一次是创建Goods对象，后面因为key值相同，所以均使用了对象池中的Goods对象。
	在这个例子中，name作为内部状态是不变的，并且作为Map的key值是可以共享的。而showGoodsPrice方法中需要传入的version值则是外部状态，它的值是变化的。
*/
